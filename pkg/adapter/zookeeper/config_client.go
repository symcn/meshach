...
func GetClient(address []string, timeout int64) (*zk.Conn, error) {
	once.Do(func() {
		rConn, _, err := zk.Connect(address, time.Duration(timeout)*time.Second)
		if err != nil {
			klog.Errorf("Get zookeeper client has an error: %v\n", err)
			return
		}
		conn = rConn
	})
