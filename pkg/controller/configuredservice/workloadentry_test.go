...
func TestReconcileConfiguredService_reconcileWorkloadEntry(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := r.reconcileWorkloadEntry(tt.args.ctx, tt.args.cr); (err != nil) != tt.wantErr {
				t.Errorf("ReconcileConfiguredService.reconcileWorkloadEntry() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
